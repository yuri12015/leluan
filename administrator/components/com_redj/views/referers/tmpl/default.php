  ...
  <thead>
    <tr>
      ...
      <th width="4%" nowrap="nowrap">
        <?php echo JHTML::_('grid.sort', 'COM_REDJ_HEADING_REFERERS_ID', 'r.id', $listDirn, $listOrder); ?>
      </th>
      <th width="35%">
        <?php echo JHTML::_('grid.sort', 'COM_REDJ_HEADING_REFERERS_VISITED_URL', 'rv.visited_url', $listDirn, $listOrder); ?>
      </th>
      <th width="35%">
        <?php echo JHTML::_('grid.sort', 'COM_REDJ_HEADING_REFERERS_REFERER_URL', 'rr.referer_url', $listDirn, $listOrder); ?>
      </th>
      <th width="6%">
        <?php echo JHTML::_('grid.sort', 'COM_REDJ_HEADING_REFERERS_DOMAIN', 'rr.domain', $listDirn, $listOrder); ?>
      </th>
      <th width="5%">
        <?php echo JHTML::_('grid.sort', 'COM_REDJ_HEADING_REFERERS_HITS', 'r.hits', $listDirn, $listOrder); ?>
      </th>
      <th width="7%">
        <?php echo JHTML::_('grid.sort', 'COM_REDJ_HEADING_REFERERS_LAST_VISIT', 'r.last_visit', $listDirn, $listOrder); ?>
      </th>
    </tr>
  </thead>
  <tfoot>
        ...
        <td>
          <?php
            if ($this->state->get('filter.decode')) {
              ...
              $tip = JText::_('COM_REDJ_RAW')."::".$item->referer_url;
            } else {
              $content = $item->referer_url;
              $tip = JText::_('COM_REDJ_DECODED')."::".urldecode($item->referer_url);
            }
          ?>
          <span style="display:block; width:400px; word-wrap:break-word;" class="editlinktip hasTip" title="<?php echo $tip; ?>"><?php echo $content; ?></span>
        </td>
        <td>
          <?php
            if ($this->state->get('filter.decode')) {
              ...
              $tip = JText::_('COM_REDJ_RAW')."::".$item->domain;
            } else {
              $content = $item->domain;
              $tip = JText::_('COM_REDJ_DECODED')."::".urldecode($item->domain);
            }
          ?>
          <span style="display:block; width:70px; word-wrap:break-word;" class="editlinktip hasTip" title="<?php echo $tip; ?>"><?php echo $content; ?></span>
        </td>
        <td>
          <?php echo $item->hits; ?>
        </td>
