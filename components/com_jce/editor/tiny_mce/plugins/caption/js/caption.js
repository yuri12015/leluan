...
var CaptionDialog = {
	...
	init : function() {
		...
		if (el != null) {
			...
			$('#border_color').val(this.getAttrib(el, 'border-color'));
			$('#align').val(this.getAttrib(el, 'align'));

			// Background Color
			$('#bgcolor').val(this.getAttrib(el, 'background-color'));
			...
			tinymce.each(ed.dom.select('div,span', el), function(c) {
				...
				$('#text_color').val(self.getAttrib(c, 'color'));
				$('#text_bgcolor').val(self.getAttrib(c, 'background-color'));
			...
			// Image Margin
			tinymce.each(['top', 'right', 'bottom', 'left'], function(o) {
				$('#margin_' + o).val(self.getAttrib(n, 'margin-' + o));
			});
			// Image Align
	...
	setBorder : function() {
		...
		$('#border~:input, #border~span').prop('disabled', !s).toggleClass('disabled', !s);
