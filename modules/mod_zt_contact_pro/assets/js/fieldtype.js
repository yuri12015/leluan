function newFieldtype()
{
	...
	var toolbar   = "<a href='javascript:newFieldtype();'>"+fieldtype_new+"</a> | <a href='javascript:deleteFieldtype("+next_inc+")'>"+fieldtype_del+"</a> <a id='newpro"+next_inc+"' href='javascript:newValue("+next_inc+")'>| "+value_new+"</a>";
	var fieldorder = '';
...
function changeElement(attribute_id, fieldType){
	...
	switch (fieldType){
		case 'textfield':
			$('newpro'+attribute_id).setStyle('opacity', '0');
		...
		break;
		case 'selected':
			$('newpro'+attribute_id).setStyle('opacity', '1');
