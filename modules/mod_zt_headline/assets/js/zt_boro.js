...
var ZTSlideBoro = new Class({
	...
	preLoad:function(){
		new Asset.images(this.arrSources,{
			onComplete:function(){
				this.arrImages.each(function(item,i){
					setTimeout(function(){	
					itemSlide = new Fx.Morph(item, {duration: 400, transition: Fx.Transitions.linear}).start({'opacity': [0,1]});},400*(i+1));					
				}.bind(this));				
	...
	initSlide:function(){
		...
		this.slides.each(function(item,i){
			...
			this.fx[i] = new Fx.Morph(this.slides[i], {duration: 400, transition: Fx.Transitions.linear});
			this.desc[i] = new Fx.Morph(this.excerpt[i],{duration:400,wait:false});
			this.headingClone[i] = new Fx.Morph(this.heading[i],{duration:400,wait:false});			
	...
	actionSlide:function(){
		this.slides.each(function(item,i){			
			item.addEvent(this.options.eventType,function(){				
				...
				this.fx[i].start({'width':this.expand_slide+(this.overlay_modifier * 1.2),'left':this.dataSlides[i].pos_active_higher});					
				this.desc[i].start({'opacity':1});
				this.headingClone[i].start({'opacity':0});
				this.slides.each(function(item1,j){	
					...
					this.fx[j].start({'width':this.minimized_slide+this.overlay_modifier,'left':new_pos});
					this.desc[j].start({'opacity':0});
					this.headingClone[j].start({'opacity':1});
					}
				}.bind(this));			
				}
				this.currentIndex = i;
	...
	setDefaultSlide:function(){
		this.slides.each(function(item,i){
			...
			this.fx[i].start({'width':this.slide_width+this.overlay_modifier,'left':new_pos});
			this.desc[i].start({'opacity':0});
			this.headingClone[i].start({'opacity':1});			
		}.bind(this));
		this.currentIndex = -1;
	}	
});
ZTSlideBoro.implement(new Events);
ZTSlideBoro.implement(new Options);