...
var ZTSlideEoty = new Class({
	...
	initialize:function(options){
		...
		desc.set('html',this.description[0].innerHTML);
		...
		this.fxDesc = new Fx.Morph(this.descXX, {duration: 500, transition: Fx.Transitions.linear});
		if(this.options.directionNav) {
			//If click pre button
	...
	runEotySlide:function(flag){
		...
		if(this.options.effect == 'sliceDownLeft' || this.options.effect == 'sliceDown' || this.options.effect == 'sliceDownRight' || this.vars.randEffect  == 'sliceDownLeft'
			...
			slides.each(function(item,i){
				this.fxItems[i] = new Fx.Morph(item, {duration: 500, transition: Fx.Transitions.linear});
			}.bind(this));
			//End init
			...
			//Init fxItems to run effect
			slides.each(function(item,i){
				this.fxItems[i] = new Fx.Morph(item, {duration: 1000, transition: Fx.Transitions.linear});
			}.bind(this));
			//End init	
			slides.each(function(item,i){				
	...
	finishFx:function(){
		...
		this.descXX.set('html',this.description[this.vars.currentSlide].innerHTML);
		this.fxDesc.start({opacity:'0.8'});
	...
	fxStart:function(index,objEffect){
		//console.log(this.fxItems[index]);
		this.fxItems[index].start(objEffect);		
	},
	stop:function(){
		$clear(this.timer);
	}
...
ZTSlideEoty.implement(new Options);
ZTSlideEoty.implement(new Events);
}
