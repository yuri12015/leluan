...
var ZTSlideIner = new Class({
	...
	initialize:function(options){		
		...
		this.loadingElement = new Element('div').addClass('loadingIner').injectTop(this.jvSlide); 
		this.loadingElement.setStyles({
			left:this.jvSlide.getSize().x/2.2-this.loadingElement.getSize().x/3,
			top:this.jvSlide.getSize().y/2.3-this.loadingElement.getSize().y/2
		});		    
        ...
        this.info = new Fx.Morph(this.jvSlide8Info[0], {duration:200});
        this.info.start({'opacity': 1});
    ...
    changeItem:function(num){
    	if (this.currentIter != num){
			...
			if ((jv_current_num>1) && ($('jv_pedon_headline'+this.options.slideId+'_content_'+jv_current_num).getAttribute('rel'))) { 
				if(this.options.linkimg>0){ 
					$('jv_pedon_headline'+this.options.slideId+'_content_'+jv_current_num).innerHTML = '<a class="link'+this.options.slideId+'_'+jv_current_num+'" href="'+$$('a.link'+this.options.slideId+'_'+jv_current_num).getProperty('href')+'">'+$('jv_pedon_headline'+this.options.slideId+'_content_'+jv_current_num).getAttribute('rel')+'</a>';
					$('jv_pedon_headline'+this.options.slideId+'_content_'+jv_current_num).removeProperty('rel');
				}else{ 
					$('jv_pedon_headline'+this.options.slideId+'_content_'+jv_current_num).innerHTML = ''+$('jv_pedon_headline'+this.options.slideId+'_content_'+jv_current_num).getAttribute('rel');
					$('jv_pedon_headline'+this.options.slideId+'_content_'+jv_current_num).removeProperty('rel');
				}
			}   
    		//this.info.stop();    			   			
			this.oldSlide = new Fx.Morph(this.jvSlide8Bg[this.currentIter], {duration:this.options.durationSlide8, wait: false});
            this.newSlide = new Fx.Morph(this.jvSlide8Bg[num],{duration:this.options.durationSlide8, wait: false});
            ZTSlideIner.Transitions[this.options.jvStyleEffect].pass([
                this.oldSlide,
                this.newSlide,
                this.currentIter,
                num], this)();
...
ZTSlideIner.implement(new Events);
ZTSlideIner.implement(new Options);

ZTSlideIner.Transitions = new Hash ({
    fade: function(oldFx, newFx, oldPos, newPos){
        ...
        else
        {
            newFx.set({opacity: 1}).chain(function(){
                this.info = new Fx.Morph(this.jvSlide8Info[newPos], {duration:200});
                this.info.start({'opacity': 1});
            }.bind(this));;
 ...
 * @author jon
 */
ZTSlideIner.Transitions.extend({
