...
switch ($layoutStyle) {
	...
	case "jv_slide11":
		...
		break;
	case "jv_slide12": 
		$moduleWidth 	= $params->get('zt_scroll_width');
		$moduleHeight 	= $params->get('zt_scroll_height');
		$imgWidth 		= $params->get('zt_scroll_thumb_width');
		$imgHeight 		= $params->get('zt_scroll_thumb_height');		
		$showButNext 	= $params->get('zt_scroll_show_next'); 
		$list = $jvCommon->getSlideContent($params);
		if(count($list)) require(JModuleHelper::getLayoutPath('mod_zt_headline','zt_scroll'));
		break;
	case "jv_slide13": 
		$moduleWidth 	= $params->get('zt_meda_width'); 
		$imgWidth 		= $params->get('zt_meda_thumb_width');
		$imgHeight 		= $params->get('zt_meda_thumb_height');		
		$showButNext 	= $params->get('zt_meda_show_next'); 
		$list = $jvCommon->getSlideContent($params);	
		if(count($list)) require(JModuleHelper::getLayoutPath('mod_zt_headline','zt_meda'));
		break;
}
?>