...
var ZT_Flow = new Class({
	...
	initialize:function(options){		
		...
		this.items.each(function(item, numb){
			this.items[numb].set('morph', {duration: 1000-(numb*100), transition: Fx.Transitions.Sine.easeOut, onComplete: function(){}});
			this.items[numb].morph({opacity: [0, (100-(numb*10))/100], width: this.options.imgwidth-(numb*32.3), height: this.options.imgheight-(numb*28.3), left: 10+((numb*70.5)/1.6), top: 5-(numb*(-10.5-(numb)))});
		}.bind(this));  
    ...
    slideContainer: function(num){
        ...
        this.items.each(function(element, n){
            ...
            if(zt_int == 50){
                ...
                this.items[n].set('morph', {duration: '800', transition: Fx.Transitions.Sine.easeOut, onComplete: function(event){}});
                this.items[n].morph({opacity: 0.5, width: [94,161.5], height: [115, 115], left: [335, 267], top: [124, 124]}); 
                ...
                this.items[n].morph({opacity: [0.8, 0.5], width: [this.options.imgwidth, ztWidth], height: [this.options.imgheight, ztHeight], left: [10, ztLeft], top: [5, ztTop]}); 
            }else{ 
                var zt_getint = 50-(zt_int+1);
                this.items[n].setStyle("z-index", zt_int+1);
                this.items[n].set('morph', {duration: '800', transition: Fx.Transitions.Sine.easeOut, onComplete: function(){}});
                this.items[n].morph({opacity: (100-(zt_getint*10))/100, width: this.options.imgwidth-(zt_getint*32.3), height: this.options.imgheight-(zt_getint*28.3), left: 10+((zt_getint*70.5)/1.6), top: 5-(zt_getint*(-10.5-(zt_getint)))});
            }
            if(this.items[n].getStyle("z-index")==50){
                this.content.each(function(element, j){
                    if(n==j){
                        zt_slideContent = new Fx.Morph(this.content[j], {duration:500, wait: false});
                        zt_slideContent.start({opacity: 1});
                    }else{
                        zt_slideContent = new Fx.Morph(this.content[j], {duration:500, wait: false});
                        zt_slideContent.start({opacity: 0});
                    }
                }.bind(this));
                this.currentIter=n;
            }
